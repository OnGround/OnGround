...
export class UserLoginServiceService {
  ...
  loginUser(usernameOrEmail: string, password: string) {
    ...
    for (let i = 0; i < this.userDetails.length; i++) {
      if (
        this.userDetails[i].username == usernameOrEmail ||
        this.userDetails[i].email == usernameOrEmail
      ) {
        if (this.userDetails[i].password == password) valid = 1;
        else wrongPassword = 1;
      }
    }
    if (wrongPassword == 1) {
